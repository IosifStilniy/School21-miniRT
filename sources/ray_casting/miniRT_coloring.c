...
/*   Updated: 2022/06/09 21:43:28 by ncarob           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_is_in_shadow(t_cart phit, t_list *object, t_light light)
{
	...
	new_ray = ft_substract_vectors(light.pos, phit);
	...
	while (object)
	{
		...
		distance_to_object = ft_get_intersection_with_sphere(new_ray,
					ft_multiply_vector(phit, 1.001f), object->content);
		...
		// 				(t_cart){0.000001f, 0.000001f, 0.000001f}), object);
...
int	ft_find_light(t_cart phit, t_obj *object, t_info *info)
{
	...
	lk[0] = ft_multiply_vectors(info->a_light.color, object->colrs);
	if (!ft_is_in_shadow(phit, info->objects, info->lights))
	...
	// if (!ft_strncmp("sp", object->identifier, 3))
	n_vect[2] = ft_substract_vectors(phit, object->crdstm.pos);
	// else
	// 		n_vect[2] = object->vector;
	n_vect[2] = ft_get_vector_norm(n_vect[2], ft_get_vector_length(n_vect[2]));
	...
	lk[1] = ft_multiply_vectors(info->lights.color, object->colrs);
