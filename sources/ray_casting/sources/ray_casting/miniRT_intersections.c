...
/*   Updated: 2022/06/26 20:45:39 by ncarob           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_check_if_inside(t_vrtx p[3], t_cart *phit, t_cart *norm, float k[3])
{
	...
	edge = ft_substract_vectors(&p[0].dot, &p[1].dot);
	vec_c = ft_substract_vectors(phit, &p[0].dot);
	edge = ft_get_cross_product(&edge, &vec_c);
	if (ft_get_dot_product(norm, &edge) > 0)
		return (0);
	if (k)
		k[1] = ft_get_vector_length(&edge);
	edge = ft_substract_vectors(&p[1].dot, &p[2].dot);
	vec_c = ft_substract_vectors(phit, &p[1].dot);
	edge = ft_get_cross_product(&edge, &vec_c);
	if (ft_get_dot_product(norm, &edge) > 0)
		return (0);
	if (k)
		k[0] = ft_get_vector_length(&edge);
	edge = ft_substract_vectors(&p[2].dot, &p[0].dot);
	vec_c = ft_substract_vectors(phit, &p[2].dot);
	edge = ft_get_cross_product(&edge, &vec_c);
	if (ft_get_dot_product(norm, &edge) > 0)
		return (0);
	return (1);
...
int	ft_intersect_triangle(t_vrtx p[3], t_cart phit, t_cart norm, float k[3])
{
	...
	edge = ft_substract_vectors(&p[1].dot, &p[0].dot);
	vec_c = ft_substract_vectors(&p[2].dot, &p[0].dot);
	edge = ft_get_cross_product(&edge, &vec_c);
	denom = ft_get_vector_length(&edge);
			...
			t_obj *object, float *closest_distance)
{
	...
	while (++i < polys.polynum)
	{
		...
		if (dist[1] < dist[0])
		{
			...
			phit = ft_multiply_vector(&ray.dir, dist[1] + 0.0001f);
			phit = ft_summ_vectors(&phit, &ray.orig);
			if (ft_intersect_triangle(p, phit, polys.poly[i].norm, k))
			{
				dist[0] = dist[1];
				*closest_norm = ft_multiply_vector(&p[0].norm, k[0]);
				edge = ft_multiply_vector(&p[1].norm, k[2]);
				*closest_norm = ft_summ_vectors(&edge, closest_norm);
				edge = ft_multiply_vector(&p[2].norm, k[1]);
				*closest_norm = ft_summ_vectors(&edge, closest_norm);
				*closest_norm = ft_get_vector_norm(closest_norm,
						ft_get_vector_length(closest_norm));
			}
		}
	}
